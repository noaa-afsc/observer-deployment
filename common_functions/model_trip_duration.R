# Trip duration model
# Relies on established database connection and both ADP_version ADPyear as inputs
# The resulting model should be applied to the primary effort object containing:
#   'RAW', as the raw difference bewteen Valhalla trip_start - trip_end
#   'AGENCY_GEAR_CODE', as a factor with levels HAL, POT, PTR, and NPT
#   'ADP', as a factor with levels including the 3 most full recent years in Valhalla

model_trip_duration <- function(ADP_version, ADPyear, use_mod="DAYS ~ RAW"){
  
  range1 <- ADPyear - 5; range2 <- ADPyear - 1
  ...
  if(exists("mod_dat", where=.GlobalEnv)==FALSE){
    ...
    o_val <- setDT(dbGetQuery(channel_afsc, paste(
      "
      SELECT DISTINCT
    ...
    n_val <- setDT(dbGetQuery(channel_afsc, paste(
      "
      SELECT DISTINCT
        a.adp, a.trip_id, a.cruise, a.permit, a.reporting_area_code AS AREA, a.agency_gear_code AS GEAR,
        a.processing_sector AS SECTOR, a.trip_target_code AS TARGET, a.trip_target_date AS TRIP_START, 
