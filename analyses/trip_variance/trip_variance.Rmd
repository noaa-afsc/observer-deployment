...
mean <- dat[, lapply(.SD, function(x) mean(x[OBSERVED_FLAG=="Y"], na.rm = TRUE)), .SDcols = c("chnk_psc", "hlbt_psc", "discard", "crab_psc"), keyby = c("STRATA", "N")]
var <- dat[, lapply(.SD, function(x) var(x[OBSERVED_FLAG=="Y"], na.rm = TRUE)), .SDcols = c("chnk_psc", "hlbt_psc", "discard", "crab_psc"), keyby = c("STRATA", "N")]
...
dat[, ":=" (STRATA = recode(STRATA, "EM_HAL" = "EM HAL", "EM_POT" = "EM POT", "EM_TRW_EFP" = "EM TRW EFP"), metric = recode(metric, "chnk_psc" = "Chinook PSC", "hlbt_psc" = "Halibut PSC", "discard" = "Groundfish discards", "crab_psc" = "Crab PSC"))]
...
I wrote a function that estimates the trip-level mean, variance, standard error, and CV when given a data set of effort, the PSC and discards associated with that effort, and the trip column that those two data sets should be joined on:  
```{r function}
effort <- copy(efrt)[ADP == 2022]
effort <- unique(work.data[, .(TRIP_ID, OBSERVED_FLAG)])[effort, on = .(TRIP_ID)]

source("common_functions/evaluate_trip_variance.R")
```

```{r example, echo=TRUE}
evaluate_trip_variance(effort, trips_melt, trip_col = "TRIP_ID")
```
