...
load("cost_weighted_boxes_exploration_Rmd.Rdata") # https://drive.google.com/file/d/1JK-8v7McDsGGbw3geON3tsXNsCjbMFvV/view?usp=drive_link
```
...
where $h$ is each stratum, $N$ is the total number of trips, $S$ is the standard deviation of a specified metric, and $c$ is the per-unit monitoring cost.
...
\
$\bar{P}_h$ is dependent on the distribution of fishing trips in space and time - specifically, how many trips are within or neighboring each box. In this sense, the spatiotemporal extent and total number of trips $N_h$ affect $\bar{P}_h$. At any given monitoring rate, if more boxes are sparsely populated, $\bar{P}_h$ will be higher. Both *EM_POT* and *EM_TRW* are small strata in terms of $N_h$, but it is clear that the distribution of fishing effort in *EM_POT* is much more diffuse than for *EM_TRW* because a much higher $r_h$ is required to reduce $\bar{P}_h$. The effect of $N_h$ is apparent in the *OB_POT* stratum, which has increased in recent years, as shown by the lower $\bar{P}_h$ values at any given $r_h$.
...
- **TO DO** : Haven't tested to see how different stratification schemes perform.
