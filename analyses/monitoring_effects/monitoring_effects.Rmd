...
dat   <- perm_results$N_table[perm_results$obs, on = .(ADP, STRATA)]
dat   <- melt(dat, id.vars = c(groups, "N", "n", "rate", "SCALE"), measure.vars = metrics, variable.name = "metric", value.name = "diff")
...
dat   <- pivot_wider(dat, names_from = SCALE, values_from = diff)
dat   <- setDT(rename(dat, raw_diff = RAW, pct_diff = PCT))

# Construct a linear model of percent difference ~ rate for each stratum and metric combination
mod <- lm(pct_diff ~ -1 + STRATA:metric + rate:STRATA:metric, data = dat)
...
mutate(STRATA = str_match(name, "STRATA\\s*(.*?)\\s*:")[,2],
       metric = str_match(name, "metric\\s*(.*?)\\s*:")[,2],
...
mod_df <- dat[setDT(mod_df), on = .(STRATA, metric)]

# Recode strata and metrics for the figure
mod_df[, ':=' (STRATA = str_replace_all(STRATA, "_", " "), 
...
# Create figure
f1 <- ggplot(mod_df[STRATA != "EM TRW EFP"], aes(x = rate * 100, y = pct_diff)) +
      ...
      labs(x = "Monitoring rate (%)", y = "Difference in means (%, monitored - unmonitored)", color = "Statistically significant?") +
...
# Save figure
if(!file.exists("output_figures/permutation_pct_diff_rate.png")){
ggsave(filename = "output_figures/permutation_pct_diff_rate.png", width = 11, height = 8.5, units = 'in')
}
```
  
Figure 1. Difference in means (%, monitored - unmonitored) against monitoring rate for years 2013 through 2021. Each point represents one year of data. Lines represent the linear regression of the points. We labeled points statistically significant if the *p*-value from the permutation test was $\le$ 0.05/6 for that year of data. We labeled lines statistically significant if the *p*-value for the linear model slope coefficient was $\le$ 0.05/6.  

```{r figure_2}
# Construct a linear model of percent difference ~ n for each stratum and metric combination
mod <- lm(pct_diff ~ -1 + STRATA:metric + n:STRATA:metric, data = dat)
...
# Create figure
f2 <- ggplot(mod_df[STRATA != "EM TRW EFP"], aes(x = n, y = pct_diff)) +
      stat_smooth(aes(color = sig_slope), method = "lm", formula = 'y ~ x', se = FALSE) +
      geom_point(aes(color = sig_point)) +
      geom_hline(yintercept = 0, lty = 3) +
      facet_grid(metric ~ STRATA, scales = "free") +
      scale_color_manual(values = c("Yes" = "red", "No" = "black")) +
      labs(x = "Sample size", y = "Difference in means (%, monitored - unmonitored)", color = "Statistically significant?") +
      theme(legend.position = "bottom")
...
# Save figure
if(!file.exists("output_figures/permutation_pct_diff_n.png")){
ggsave(filename = "output_figures/permutation_pct_diff_n.png", width = 11, height = 8.5, units = 'in')
}
```
  
Figure 2. Difference in means (%, monitored - unmonitored) against sample size for years 2013 through 2021. Each point represents one year of data. Lines represent the linear regression of the points. We labeled points statistically significant if the *p*-value from the permutation test was $\le$ 0.05/6 for that year of data. We labeled lines statistically significant if the *p*-value for the linear model slope coefficient was $\le$ 0.05/6.

```{r figure_3}
# Wrangle data for figure
dat <- mod_df %>% 
       select(STRATA, metric, pct_diff) %>% 
       group_by(STRATA, metric) %>% 
       summarise(mean = mean(pct_diff), 
                    se = sd(pct_diff) / sqrt(n()), 
                    lci = mean - (qnorm(.025/6,lower.tail=FALSE) * se), 
                    uci = mean + (qnorm(.025/6,lower.tail=FALSE) * se), 
                    sig = ifelse(lci <= 0 & uci >= 0, "No", "Yes"),
                    .groups = "drop") %>% 
       setDT()

# Create figure
f3 <- ggplot(dat[STRATA != "EM TRW EFP"], aes(x = metric, y = mean, color = sig)) +
      geom_point() +
      geom_errorbar(aes(ymin = lci, ymax = uci)) +
      geom_hline(yintercept = 0, lty = 3) +
      facet_grid(. ~ STRATA) +
      scale_color_manual(values = c("Yes" = "red", "No" = "black")) +
      labs(x = "Metric", y = "Mean difference in means (%, monitored - unmonitored)", color = "Statistically significant?") +
...
# Save figure
if(!file.exists("output_figures/permutation_pct_diff_summary.png")){
ggsave(filename = "output_figures/permutation_pct_diff_summary.png", width = 11, height = 8.5, units = 'in')
}
```

Figure 3. Mean difference in means (%, monitored - unmonitored) across all years of data from 2013 through 2021. Error bars represent 95% confidence intervals. We labelled results statistically significant if the 95% confidence interval does not contain 0.

```{r figure_4}
# Wrangle data for figure
dat <- mod_df %>% 
       select(STRATA, metric, raw_diff) %>% 
       group_by(STRATA, metric) %>% 
       summarise(mean = mean(raw_diff), 
                    se = sd(raw_diff) / sqrt(n()), 
                    lci = mean - (qnorm(.025/6,lower.tail=FALSE) * se), 
                    uci = mean + (qnorm(.025/6,lower.tail=FALSE) * se), 
                    sig = ifelse(lci <= 0 & uci >= 0, "No", "Yes"),
                    .groups = "drop") %>% 
...
# Create figure
f4 <- ggplot(dat[STRATA != "EM TRW EFP"], aes(x = STRATA, y = mean, color = sig)) +
      geom_point() +
      geom_errorbar(aes(ymin = lci, ymax = uci)) +
      geom_hline(yintercept = 0, lty = 3) +
...
# Save figure
if(!file.exists("output_figures/permutation_raw_diff_summary.png")){
ggsave(filename = "output_figures/permutation_raw_diff_summary.png", width = 11, height = 8.5, units = 'in')
}
```

Figure 4. Mean difference in means (monitored - unmonitored) across all years of data from 2013 through 2021. Error bars represent 95% confidence intervals. We labelled results statistically significant if the 95% confidence interval does not contain 0.
