...
```{r figure_1}
# Get p-values and monitoring rates from the permutation test results
dat <- perm_results$N_table[perm_results$`p-value`, on = .(ADP, STRATA)]
dat <- melt(dat, id.vars = c(groups, "N", "n", "rate"), measure.vars = metrics, variable.name = "metric", value.name = "p_val")

# Construct a linear model of p-value ~ rate for each stratum and metric combination
mod <- lm(p_val ~ -1 + STRATA:metric + rate:STRATA:metric, data = dat)

# Extract the slope coefficients from the linear model
mod_df <- data.frame(name = names(mod$coefficients), coefficient = unname(mod$coefficients))
mod_df <- mod_df %>% 
filter(grepl("rate", name)) %>% 
mutate(STRATA = str_match(name, "STRATA\\s*(.*?)\\s*:metric")[,2],
       metric = str_match(name, "metric\\s*(.*?)\\s*:rate")[,2],
       # If p-values from the permutation test are correlated with 
       # monitoring rate, we would expect the correlation to be positive
       expected = ifelse(coefficient > 0, "Yes", "No")) %>% 
select(STRATA, metric, slope = coefficient, expected)
...
# Create figure 
f1 <- ggplot(dat, aes(x = rate, y = p_val, color = expected)) +
      geom_point() +
      stat_smooth(method = "lm", formula = 'y ~ x', se = FALSE) +
      facet_grid(metric ~ STRATA, scales = "free") +
      scale_x_continuous(labels = number_format(accuracy = 0.01)) +
      labs(x = "Monitoring rate", y = "p-value", color = "Were monitoring effects reduced (as measured by a linear \nincrease in p-value) as monitoring rate increased?") +
...
# Save figure
if(!file.exists("output_figures/permutation_p_val.png")){
ggsave(filename = "output_figures/permutation_p_val.png", width = 11, height = 8.5, units = 'in')
}
```
  
Figure 1. The *p*-values from permutation tests against realized monitoring rates. Each point represents one year of data on which the permutation test was run. Partial coverage data from 2013 through 2021 were used. Lines represent the linear regression of the points.  
...
mutate(STRATA = str_match(name, "STRATA\\s*(.*?)\\s*:metric")[,2],
       metric = str_match(name, "metric\\s*(.*?)\\s*:rate")[,2],
       # If percent differences are correlated with monitoring rate, 
       # we would expect the correlation to be negative
       expected = ifelse(coefficient < 0, "Yes", "No")) %>% 
select(STRATA, metric, slope = coefficient, expected)
...
# Create figure
f2 <- ggplot(dat, aes(x = rate, y = pct_diff, color = expected)) +
      geom_point() +
      stat_smooth(method = "lm", formula = 'y ~ x', se = FALSE) +
      facet_grid(metric ~ STRATA, scales = "free") +
      labs(x = "Monitoring rate", y = "% Difference in means (monitored - unmonitored)", color = "Were monitoring effects reduced (as measured by a linear \ndecrease in % difference) as monitoring rate increased?") +
      theme(legend.position = "bottom")
...
Figure 1. The percent differences in means (monitored - unmonitored) against realized monitoring rates. Each point represents one year of data. Partial coverage data from 2013 through 2021 were used. Lines represent the linear regression of the points.    